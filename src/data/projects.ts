...
export const projects: ProjectData[] = [
  ...
  },
  {
    ...
    featured: true,
    impact: "Full Claude Code alternative built from scratch in TypeScript — 17 tools, 28 slash commands, 11 providers, React Ink TUI, and a self-learning engine. MiniMax M2.7 integration delivers near-Sonnet coding quality at ~90% lower cost.",
    problem:
      "Claude Code locks you into Anthropic. Cursor locks you into a GUI. When a model goes down, hits rate limits, or costs too much for a task, there is no fallback. Teams running intensive agentic sessions can hit $100+/month per developer with no way to route cheaper models for simpler work or switch providers without changing tools.",
    solution:
      "A TypeScript CLI agent with full provider independence — 11 LLM providers through one unified streaming interface. Anthropic, OpenAI, MiniMax, Groq, Google, DeepSeek, Mistral, xAI, OpenRouter, Together, Ollama. Auto-discovers the best available model. Smart routing picks the right model per prompt type. MiniMax M2.7 delivers near-Sonnet quality at ~90% lower cost. Anthropic tool search (defer_loading) automatically cuts context overhead by ~85% when tool count exceeds 10. Built across 9 sessions covering UI, engine reliability, context intelligence, and production safety.",
    architecture: [
      ...
      "Context Manager (js-tiktoken exact counting, semantic compaction via active model at 85%)",
      "Session Layer (WAL persistence, save/resume by ID, crash recovery)",
    ...
    features: [
      "11 LLM providers with unified streaming — Anthropic, OpenAI, MiniMax, Groq, Google, DeepSeek, Mistral, xAI, OpenRouter, Together, Ollama — auto-discovered from env vars at startup",
      "Smart auto-routing: analyzes each prompt and routes to the best available model (code → fast model, reasoning → deep model, simple → free model)",
      "MiniMax M2.7: OpenAI-compatible API, 1M context window, ~90% cheaper than Claude Sonnet, outperforms Sonnet 4.5 on SWE-bench Multilingual",
      "Anthropic tool search: BM25 deferred loading auto-enabled above 10 tools — core tools (file, bash, grep, glob) always loaded, 11 secondary tools discovered on demand, ~85% context reduction",
      ...
      "Sub-agent system: spawn specialized sub-agents for research tasks, each with independent model selection and context",
      ...
      "Status bar: SHAMI › path › branch~dirty | context% › $cost › time — updates live during streaming",
      "Git integration: /git status/diff/log, /commit, /pr — create commits and PRs from chat",
      "Task management: /tasks add/done — persistent task board across sessions",
      ...
      "Plugin system: load external tools from ~/.gogaa/plugins/ — each plugin registers tools and hooks",
      "MCP support: connect external Model Context Protocol servers, tools appear dynamically in the registry",
      "5-strategy JSON arg parser: handles truncated, escaped, and malformed JSON from model output — ~95% parse success vs ~70% with naive JSON.parse",
      "Bash sandboxing: destructive command denylist + path allowlist + full audit log at ~/.gogaa/audit/{session}.jsonl",
      "Session persistence with WAL crash recovery — resume any past conversation by ID, zero work lost on network failures or model switches",
      "Self-learning engine: tracks tool reliability, error patterns, and model performance across sessions — auto-improves tool descriptions and routing preferences over time",
      "Cost budget enforcement: configurable $ cap per session, warns at 80%, hard stops at 100%",
      "Plan mode: switches all tools to read-only for safe exploration before committing to changes",
      "Voice input: /voice transcribes spoken prompts via Groq or OpenAI Whisper",
      "Context compaction: js-tiktoken exact token counting per model, semantic summarization via active model when context hits 85%",
    ],
    techDecisions: [
      {
        title: "5-strategy JSON arg parser instead of naive JSON.parse",
        description: "Models frequently return malformed tool arguments — truncated strings, unescaped characters, extra whitespace, or partial JSON. A single JSON.parse fails silently or throws. The 5-strategy parser tries progressively looser parsing (strict → trim → unescape → partial reconstruct → fallback object) and injects error recovery hints into tool results when it partially recovers. Success rate went from ~70% to ~95% on real agentic runs.",
      },
      {
        title: "MiniMax over OpenRouter for cost optimization",
        description: "MiniMax M2.7 has a direct API (not aggregated through a router), a 1M context window, an OpenAI-compatible endpoint requiring zero extra code, and consistently outperforms Claude Sonnet 4.5 on SWE-bench Multilingual. Aggregators like OpenRouter add latency and markup. A direct integration gives better pricing, lower latency, and a stable endpoint.",
      },
      {
        title: "Anthropic tool search (defer_loading) for context efficiency",
        description: "Loading all 17 tool definitions upfront costs 10–15k context tokens before Claude does any work. The Anthropic BM25 tool search tool defers non-core tools and loads only the 3–5 relevant ones per request. Core tools (file-read/write/edit, bash, grep, glob) are always present since they're needed in nearly every turn. The remaining 11 are deferred — saving ~45k tokens over a typical agentic session.",
      },
      {
        title: "React Ink for TUI instead of raw ANSI escape codes",
        description: "Raw ANSI requires manual cursor positioning, buffering, and diff logic for partial re-renders — fragile and hard to extend. React Ink gives a component model where each UI element (streaming output, permission prompt, status bar, background job list) is an independent stateful component. Updates re-render only what changed. Adding a new UI element is a new component, not another block of escape code.",
      },
      {
        title: "WAL persistence for session recovery",
        description: "Sessions are written to a write-ahead log incrementally as messages are added, not serialized in one shot at the end. If the process crashes mid-session, the last complete state is always recoverable. Resume by ID works across model switches, reboots, and network failures without replaying the full conversation from scratch.",
      },
    ],
    stack: ["TypeScript", "Node.js", "React Ink", "Anthropic SDK", "OpenAI SDK", "js-tiktoken", "MiniMax", "Groq", "Google Gemini", "Ollama"],
    results: [
      "17 tools, 28 slash commands, 11 providers — full Claude Code feature parity built from scratch in TypeScript",
      "Smart routing + MiniMax M2.7 cuts per-session cost by ~90% vs Anthropic Sonnet with no regression on coding benchmarks",
      "Anthropic tool search reduces context overhead by ~85% on large tool sets — 45k+ tokens saved per agentic session",
      "5-strategy JSON parser brings tool call success rate from ~70% to ~95% on real model output",
      "All 29 tests passing, 8 security audit findings resolved, full bash sandboxing + per-session audit log",
      "Session persistence with WAL crash recovery — zero lost work across model switches or failures",
    ],
