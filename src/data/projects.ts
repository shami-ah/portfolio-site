...
export const projects: ProjectData[] = [
  {
    ...
    type: "Open Source",
    impact: "Reviews code in under 1 second with zero runtime dependencies, running entirely on your machine.",
    problem:
      "Commercial code review tools are slow, cloud-dependent, expensive, and do either pattern matching or AI reasoning. Never both. Teams need fast, private, accurate reviews without sending their code to third-party servers.",
    solution:
      "A hybrid review engine that combines 113 deterministic patterns with AI reasoning in a 5-pass pipeline. It builds a persistent codebase index on first run, then does incremental updates in 60ms for instant reviews on every save.",
    architecture: [
      "Git Diff",
      "AST-aware File Parser",
      "Persistent Index (call graph + schema graph + type graph + column registry)",
      "Incremental Update (60ms)",
      "5-Pass Review Engine",
      "AI Reasoning Layer (Claude / Codex / Gemini)",
      "Self-Learning Feedback Loop",
    ],
    features: [
      "5-pass review: pre-flight, layer analysis, cross-file tracing, pattern scan, self-validation",
      "113 patterns across 4 stacks with OWASP Top 10 and CWE mapping",
      "Persistent codebase index with 62K+ edges in the call graph",
      "Self-learning noise filter using TF-IDF similarity with zero external dependencies",
    ...
    techDecisions: [
      ...
      },
      {
        ...
        description: "Deterministic patterns catch known issues instantly. AI reasoning layer catches semantic problems that no pattern can express. Best of both worlds.",
      },
      {
    ...
    stack: ["TypeScript", "Regex Parsers", "Persistent JSON Index", "GitHub Actions", "Claude Code Adapter"],
    results: [
