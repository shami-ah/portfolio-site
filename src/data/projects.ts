...
export const projects: ProjectData[] = [
  {
    ...
    oneLiner: "430 hand-crafted bug patterns across 9 stacks. Under one second. Entirely on your machine.",
    ...
    architecture: [
      ...
      "Pass 3: Pattern Scan (~430 patterns, 9 stacks) + AI Agent Auto-Detection",
    ...
    features: [
      "~430 hand-crafted patterns across 9 stacks (TypeScript, Python, Go, Java, Ruby, PHP, Next.js, FastAPI, Spring Boot). Every rule maps to a real production failure mode, tagged with OWASP/CWE",
    ...
    techDecisions: [
      ...
      },
      {
        ...
        description: "Static rule sets go stale. I built a pipeline called Glean that processes real PR reviews from production codebases, extracts generalizable bug patterns, and deduplicates them against the existing library. Over 520 PRs from a production SaaS with 40+ edge functions, Stripe Connect, and Supabase have been processed. The pipeline has added 194 patterns to the react-supabase-ts module alone. Each new project I review makes the tool better for every project after it.",
      },
      {
    ...
    results: [
      "~430 patterns across 9 stacks, every rule validated against real production repos",
      ...
      "Glean pipeline has processed 600+ production PRs, extracting 194 novel patterns into the react-supabase-ts module. Agent Harvest added 34 patterns in its first sweep. Greptile Parity benchmarking raised coverage from 8% to 67% on a real production PR in one session",
    ],
    ...
    decision: {
      ...
      myChoice: 2,
      reasoning:
        "ESLint stays single-file. Commercial SaaS sends your code to a third party and still misses schema/cross-file bugs. I built a hybrid engine: ~430 hand-crafted patterns + persistent call graph + source-to-sink taint tracer + agent harvest pipeline. Runs fully local, zero deps, <1s reviews. Now active in production.",
    },
    vs: {
      mine: {
        title: "CodeLens",
        bullets: [
          "~430 hand-crafted patterns validated against real failures",
  ...
  },
  {
    ...
    oneLiner: "AI turns daily client emails into approved bookings, invoices, and CRM updates.",
  ...
  },
  {
    ...
    oneLiner: "Complete visibility into every AI coding session. Local-first. Your data never leaves your machine.",
    ...
    architecture: [
      ...
      "Quality Scoring Engine (grades sessions A-F based on efficiency, focus, cost, self-correction, and retries)",
    ...
    features: [
      ...
      "X-Ray view: action-by-action session replay showing every tool call with CAMEL-aligned phase labels (planning, exploring, executing, verifying, refining), timing, file focus areas, session health blocks, and error tracking. Detects whether the AI is using multi-pass reasoning or single-shotting",
      "Session Quality grading: A-F scores based on 6 factors including self-correction detection (edit-test-fix cycles). CAMEL-aligned phase detection (planning, exploring, executing, verifying, refining) with coaching for missing verification. Grade distribution chart, top sessions vs needs-improvement leaderboard",
  ...
  },
  {
    ...
    subtitle: "CAMEL Multi-Agent Framework with Iterative Critique (v0.1.0)",
    type: "AI Framework",
    impact: "A production-grade multi-agent orchestration framework that implements the CAMEL pattern: Planner decomposes goals into task DAGs, Workers execute with tool access, and a Validator runs iterative critique loops (up to 3 rounds) with structured feedback fed back into the Worker. Failed critiques are persisted to memory so future tasks learn from past mistakes. Human-in-the-loop approval gates pause execution for plan review before any work begins. 18 files, clean architecture, fully open-source.",
    problem: "Most AI agent frameworks are either toy demos (single LLM call wrapped in a class) or massive dependency trees (LangChain, CrewAI) that obscure what's actually happening. I needed a framework that was small enough to understand completely, production-grade enough to handle real tasks, and architected cleanly enough to serve as both a working tool and a portfolio piece that demonstrates genuine multi-agent engineering.",
    solution: "Built a focused orchestrator with four agent roles (Planner, Worker, Validator, Orchestrator), a formal task state machine (created, planned, pending_approval, in_progress, validating, completed), and CAMEL-inspired iterative critique. The Validator doesn't just approve or reject. It generates structured feedback that gets injected into the Worker's next attempt, creating a refinement loop that converges on quality. The HITL approval gate uses state-based pausing (not modal popups), so tasks can resume from exactly where they stopped. Memory stores error patterns indexed by agent and task type for cross-task learning.",
    architecture: [
      "Orchestrator Core (DAG-based task execution with dependency resolution)",
      "Planner Agent (goal decomposition into structured subtask DAG)",
      "Worker Agent (tool-use loop with max 10 iterations, 4 built-in tools)",
      "Validator Agent (iterative critique: up to 3 rounds of feedback and rework)",
      "Task State Machine (6 states, validated transitions, prevents invalid state changes)",
      "Event Bus (typed EventEmitter for observability: task:critique, task:validated, etc.)",
      "Memory Store (semantic recall, error pattern persistence across tasks)",
      "Approval Gate (state-based HITL: pending_approval pauses execution, CLI approve/reject)",
      "Tool Registry (Zod-validated input/output schemas per tool)",
      "Provider Factory (Claude and OpenAI adapters, model-agnostic agent definitions)",
    ],
    features: [
      "Iterative CAMEL critique loop: Validator rejects with structured feedback, Worker retries with specific fix instructions, up to 3 rounds. Converges on quality instead of single-pass approve/reject",
      "Error pattern learning: failed critiques stored in memory indexed by task type. Future tasks see past mistakes and avoid repeating them",
      "Think/Act/Reflect lifecycle: every agent thinks (plan without tools), acts (execute with tools), and reflects (store learnings). Not just a chat wrapper",
      "Task DAG execution: Planner creates subtasks with explicit dependencies. Orchestrator executes in topological order, parallelizable where dependencies allow",
      "Human-in-the-loop approval: state-based gate shows the full plan with agent reasoning and risk level before any execution begins. CLI commands to approve/reject",
      "Typed event system: task:created, task:planned, task:claimed, task:critique, task:validated, task:completed, task:failed. Every state change is observable",
      "4 built-in tools: web_search, file_read, file_write, shell_exec. Zod schemas for validated inputs and outputs",
      "Multi-provider: Claude (primary) and OpenAI adapters. Agent definitions are model-agnostic",
    ],
    techDecisions: [
      {
        title: "Iterative critique over single-pass validation",
        description: "The original validator was approve/reject with a feedback string that went nowhere. The CAMEL upgrade pipes that feedback directly into the Worker's next attempt as explicit fix instructions. This transforms validation from a gate into a refinement loop. On complex tasks, the first attempt is often 70% right. Three rounds of targeted feedback get it to 95% without starting over.",
      },
      {
        title: "State machine over ad-hoc status tracking",
        description: "Every task transition (created to planned, planned to pending_approval, etc.) goes through a validated state machine that rejects invalid transitions. This prevents the orchestrator from entering impossible states like 'completed but never executed' or 'in_progress but not approved'. It's 39 lines of code that eliminates an entire category of bugs.",
      },
      {
        title: "Memory persistence for cross-task learning",
        description: "When a task fails validation, the critique is stored in memory with the task type as the key. The next time a similar task runs, the Worker's context includes past failure patterns. The agent literally learns from its mistakes across runs, not just within a single conversation.",
      },
      {
        title: "18 files, zero unnecessary abstraction",
        description: "The entire framework is 18 files. Every file has one clear responsibility. No dependency injection framework, no plugin system, no config file format. You can read the whole codebase in an hour and understand every decision. This is deliberate: agent frameworks should be transparent, not opaque.",
      },
    ],
    stack: ["TypeScript", "Node.js", "Anthropic SDK", "OpenAI SDK", "Zod", "nanoid"],
    results: [
      "Clean 18-file architecture: orchestrator, 3 agent types, state machine, event bus, memory, tools, providers",
      "CAMEL critique loop: iterative refinement with structured feedback, up to 3 rounds per task",
      "Cross-task learning: error patterns persist in memory, improving quality on subsequent runs",
      "Fully open-source and public: the only agent repo on the profile that recruiters can clone and read",
      "State machine prevents invalid task transitions with zero runtime overhead",
    ],
