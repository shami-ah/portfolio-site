  ...
  return (
    <AnimatePresence>
      {visible && (
        ...
        >
          {/* Background — fades during exit to reveal hero behind */}
          <motion.div
            className="absolute inset-0 bg-background"
            animate={{ opacity: isExit ? 0 : 1 }}
            transition={isExit ? { duration: 0.9, delay: 0.15, ease: "easeOut" } : { duration: 0 }}
          />

          {/* Backdrop grid — fades with background */}
          <motion.div
            className="absolute inset-0 pointer-events-none"
            animate={{ opacity: isExit ? 0 : 0.05 }}
            transition={isExit ? { duration: 0.5 } : { duration: 0 }}
          >
            <div
          ...
          {/* Center glow */}
          <motion.div
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-accent/[0.05] rounded-full blur-3xl pointer-events-none"
            animate={{ opacity: isExit ? 0 : 1 }}
            transition={isExit ? { duration: 0.5 } : { duration: 0 }}
          />

          {/* Terminal — shrinks in place → becomes bubble → travels to agent */}
          <div className="absolute inset-0 flex items-center justify-center px-4">
            <motion.div
              ...
              animate={
                isExit
                  ? {
                      scale: [1, 0.38, 0.09, 0.075, 0.06],
                      borderRadius: ["12px", "28px", "50%", "50%", "50%"],
                      y: [0, 0, 0, dist, dist],
                      ...
                      boxShadow: [
                        ...
                        `0 0 50px ${hexToRgba(ac, 0.5)}, 0 0 100px ${hexToRgba(ac, 0.15)}`,
                        `0 0 45px ${hexToRgba(st, 0.5)}, 0 0 80px ${hexToRgba(st, 0.2)}`,
                        `0 0 40px ${hexToRgba(st, 0.6)}, 0 0 70px ${hexToRgba(st, 0.25)}`,
            ...
            >
              {/* Content wrapper — fades early so the shrinking box is just a glowing shell */}
              <motion.div
                animate={{ opacity: isExit ? 0 : 1 }}
                transition={{ duration: isExit ? 0.3 : 0 }}
              >
                {/* Window chrome */}
